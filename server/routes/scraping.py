...
def updateNBA(soup: BeautifulSoup):
    ...
    for i, player in enumerate(players):
        ...
        obj["hof"] = hof
        obj["name"] = header.text[0 : len(header.text) - 1] if hof else header.text
        ...
        bling = (
            info.find(id="bling").find_all("a") if info.find(id="bling") != None else []
        )
        for b in bling:
            if (
                b.text.find("MVP") > 0
                and b.text.find("Finals MVP") < 0
                and b.text.find("AS") < 0
            ):
                obj["mvps"] = int(b.text.split("x")[0]) if b.text.find("x") > 0 else 1
        ...
        cur.execute(f"SELECT id from \"ATHLETES\" where link = '{obj.get('link')}'")
        id = cur.fetchall()[0][0]
        cur.execute(
...
def updateMLB(soup: BeautifulSoup):
    ...
    for i, player in enumerate(players):
        if i == 0:
            continue
        obj = {"league": "MLB"}
        obj["link"] = "https://www.baseball-reference.com" + player.find("a")["href"]
        ...
        if not obj["active"]:
            ...
            mlbPlayer = True
        obj["name"] = unidecode(player.find("a").text).replace("'", "").replace(".", "")
        if not mlbPlayer:
            continue
        ...
        for award in arr:
            if award.text.find("All-Star") > 0:
                ...
                obj["cyyoungs"] = (
                    int(award.text.split("x")[0]) if award.text.find("x") > 0 else 1
                )
                ...
                award.text.find("MVP") > 0
                and award.text.find("WS") < 0
                and award.text.find("AS") < 0
            ):
                obj["mvps"] = (
                    int(award.text.split("x")[0]) if award.text.find("x") > 0 else 1
                )
        ...
        obj["image"] = (
            obj["image"].replace("'", "//x27") if obj["image"] != None else None
        )
        cur.execute(
            f"INSERT into \"ATHLETES\" (name, link, league, image, \"from\", \"to\", active, hof) values ('{obj.get('name')}', '{obj.get('link')}', '{obj.get('league')}', '{obj.get('image')}', {obj.get('from')}, {obj.get('to')}, {obj.get('active')}, {obj.get('hof')})"
        )
...
if __name__ == "__main__":
    ...
    for letter in alphabet:
        if len(argv) == 1:
            soup = getSoup("https://www.basketball-reference.com/players/" + letter)
            updateNBA(soup)
            soup = getSoup(
                "https://www.pro-football-reference.com/players/" + letter.capitalize()
            )
            updateNFL(soup)
            soup = getSoup("https://www.baseball-reference.com/players/" + letter)
            updateMLB(soup)
        elif argv[1] == "NBA":
            soup = getSoup("https://www.basketball-reference.com/players/" + letter)
            updateNBA(soup)
